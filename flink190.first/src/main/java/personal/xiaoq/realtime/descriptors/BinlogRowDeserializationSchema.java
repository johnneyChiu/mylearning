...
import static personal.xiaoq.realtime.descriptors.MyTimeFormats.*;

/**
 * @Author: qiuqiangqiang
 * @Email: johnney_chiu@163.com
 * @Description:
 * @Date: Created on 2019-12-02 11:01
 * @Version: V1.0.0
 */
public class BinlogRowDeserializationSchema implements DeserializationSchema<Row> {
    ...
    private BinlogRowDeserializationSchema.DeserializationRuntimeConverter runtimeConverter;

    private BinlogRowDeserializationSchema(
    ...
    /**
     * @deprecated Use the provided {@link BinlogRowDeserializationSchema.Builder} instead.
     */
    @Deprecated
    public BinlogRowDeserializationSchema(TypeInformation<Row> typeInfo) {
        this(typeInfo, false);
    }
     ...
     */
    @Deprecated
    public BinlogRowDeserializationSchema(String jsonSchema) {
    ...
    public static class Builder {
         ...
         */
        public BinlogRowDeserializationSchema.Builder failOnMissingField() {
            this.failOnMissingField = true;
            return this;
        }

        public BinlogRowDeserializationSchema build() {
            return new BinlogRowDeserializationSchema(typeInfo, failOnMissingField);
        }
    }
    ...
    public boolean equals(Object o) {
        ...
        final BinlogRowDeserializationSchema that = (BinlogRowDeserializationSchema) o;
    ...
    private BinlogRowDeserializationSchema.DeserializationRuntimeConverter createConverter(TypeInformation<?> typeInfo) {
        BinlogRowDeserializationSchema.DeserializationRuntimeConverter baseConverter = createConverterForSimpleType(typeInfo)
                .orElseGet(() ->
    ...
    private BinlogRowDeserializationSchema.DeserializationRuntimeConverter wrapIntoNullableConverter(BinlogRowDeserializationSchema.DeserializationRuntimeConverter converter) {
    ...
    private Optional<BinlogRowDeserializationSchema.DeserializationRuntimeConverter> createContainerConverter(TypeInformation<?> typeInfo) {
    ...
    private BinlogRowDeserializationSchema.DeserializationRuntimeConverter createByteArrayConverter() {
        return (mapper, jsonNode) -> {
            try {
    ...
    private BinlogRowDeserializationSchema.DeserializationRuntimeConverter createObjectArrayConverter(TypeInformation elementTypeInfo) {
        BinlogRowDeserializationSchema.DeserializationRuntimeConverter elementConverter = createConverter(elementTypeInfo);
        return assembleArrayConverter(elementConverter);
    }

    private BinlogRowDeserializationSchema.DeserializationRuntimeConverter createRowConverter(RowTypeInfo typeInfo) {
        List<BinlogRowDeserializationSchema.DeserializationRuntimeConverter> fieldConverters = Arrays.stream(typeInfo.getFieldTypes())
    ...
    private BinlogRowDeserializationSchema.DeserializationRuntimeConverter createFallbackConverter(Class<?> valueType) {
        return (mapper, jsonNode) -> {
    ...
    private Optional<BinlogRowDeserializationSchema.DeserializationRuntimeConverter> createConverterForSimpleType(TypeInformation<?> simpleTypeInfo) {
    ...
    private BinlogRowDeserializationSchema.DeserializationRuntimeConverter createDateConverter() {
        return (mapper, jsonNode) -> Date.valueOf(ISO_LOCAL_DATE.parse(jsonNode.asText())
                .query(TemporalQueries.localDate()));
    }

    private BinlogRowDeserializationSchema.DeserializationRuntimeConverter createTimestampConverter() {
        return (mapper, jsonNode) -> {
    ...
    private BinlogRowDeserializationSchema.DeserializationRuntimeConverter createTimeConverter() {
        return (mapper, jsonNode) -> {
    ...
    private BinlogRowDeserializationSchema.DeserializationRuntimeConverter assembleRowConverter(
            String[] fieldNames,
            List<BinlogRowDeserializationSchema.DeserializationRuntimeConverter> fieldConverters) {
        return (mapper, jsonNode) -> {
    ...
    private Object convertField(
            ObjectMapper mapper,
            BinlogRowDeserializationSchema.DeserializationRuntimeConverter fieldConverter,
            String fieldName,
            JsonNode field) {
        if (field == null) {
    ...
    private BinlogRowDeserializationSchema.DeserializationRuntimeConverter assembleArrayConverter(BinlogRowDeserializationSchema.DeserializationRuntimeConverter elementConverter) {
        return (mapper, jsonNode) -> {
